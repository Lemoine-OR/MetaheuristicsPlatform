...
public sealed class FullyInformedPsoScientificTests
{
    [Fact]
    public void DescriptorFactoryAndDoiUseCanonicalIdentity()
    {
        var optimizer = new FullyInformedParticleSwarmOptimizer();
        Assert.Equal(MetaheuristicAlgorithmIds.FullyInformedParticleSwarm, optimizer.Descriptor.Id);
        Assert.Contains(optimizer.Descriptor.References, reference => reference.Doi == "10.1109/TEVC.2004.826074");
        Assert.NotNull(MetaheuristicFactory.Create<FullyInformedParticleSwarmOptimizer>(MetaheuristicAlgorithmIds.FullyInformedParticleSwarm));
    }
    ...
    [Fact]
    public void UnweightedFipsDividesTotalAccelerationAcrossInformers()
    {
        Assert.Equal(0.82, FullyInformedPsoKernel.CoefficientPerInformer(4.10, 5), 12);
    }

    private static OptimizationResult<double[]> Run(ulong seed) =>
        new FullyInformedParticleSwarmOptimizer().Optimize(
            CreateSphere(6),
            new FullyInformedPsoParameters { SwarmSize = 6, MaximumIterations = 2 },
