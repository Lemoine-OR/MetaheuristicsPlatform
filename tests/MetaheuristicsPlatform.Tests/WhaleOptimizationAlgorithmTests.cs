...
public sealed class WOAScientificTests
{
    [Fact]
    public void DescriptorAndFactoryUseCanonicalScientificIdentity()
    {
        var optimizer = new WhaleOptimizationAlgorithmOptimizer();
        Assert.Equal(MetaheuristicAlgorithmIds.WhaleOptimizationAlgorithm, optimizer.Descriptor.Id);
        Assert.Contains(optimizer.Descriptor.References, reference => reference.Doi == "10.1016/j.advengsoft.2016.01.008");
        Assert.NotNull(MetaheuristicFactory.Create<WhaleOptimizationAlgorithmOptimizer>(MetaheuristicAlgorithmIds.WhaleOptimizationAlgorithm));
    }
    ...
    public void OneCompleteIterationHasValidatedEvaluationAccounting()
    {
        OptimizationResult<double[]> result =
            new WhaleOptimizationAlgorithmOptimizer().Optimize(
                CreateSphere(4),
                new WhaleOptimizationAlgorithmParameters { PopulationSize = 6, MaximumIterations = 1 },
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new WhaleOptimizationAlgorithmParameters { PopulationSize = 1 }.Validate());
    }

    private static OptimizationResult<double[]> Run(ulong seed) =>
        new WhaleOptimizationAlgorithmOptimizer().Optimize(
            CreateSphere(5),
            new WhaleOptimizationAlgorithmParameters { PopulationSize = 6, MaximumIterations = 2 },
            new ArraySolutionCloner<double>(),
            new NeverStoppingCriterion(),
