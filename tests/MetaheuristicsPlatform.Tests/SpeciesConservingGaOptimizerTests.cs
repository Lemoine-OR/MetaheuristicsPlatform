...
public sealed class SpeciesConservingGaOptimizerTests
{
    [Fact]
    public void Optimize_ReturnsDistinctOptima_AndFactoryCreatesCanonicalType()
    {
        ...
        MultimodalOptimizationResult result =
            new SpeciesConservingGaOptimizer().Optimize(
                problem,
                new SpeciesConservingGaParameters { MaximumGenerations = 4, PopulationSize = 24 },
        ...
        Assert.IsType<SpeciesConservingGaOptimizer>(
            MetaheuristicFactory.Create<SpeciesConservingGaOptimizer>(
