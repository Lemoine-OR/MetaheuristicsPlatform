...
public sealed class HomomorphousMappingEaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new HomomorphousMappingEaOptimizer().Optimize(problem,new HomomorphousMappingEaParameters { MaximumGenerations = 4 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<HomomorphousMappingEaOptimizer>(MetaheuristicFactory.Create<HomomorphousMappingEaOptimizer>(MetaheuristicAlgorithmIds.HomomorphousMappingEa));
 }
