...
public sealed class JoinesHouckPenaltyGaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new JoinesHouckPenaltyGaOptimizer().Optimize(problem,new JoinesHouckPenaltyGaParameters { MaximumGenerations = 4 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<JoinesHouckPenaltyGaOptimizer>(MetaheuristicFactory.Create<JoinesHouckPenaltyGaOptimizer>(MetaheuristicAlgorithmIds.JoinesHouckPenaltyGa));
 }
