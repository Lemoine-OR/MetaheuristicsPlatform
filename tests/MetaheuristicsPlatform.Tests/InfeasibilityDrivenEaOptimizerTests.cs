...
public sealed class InfeasibilityDrivenEaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new InfeasibilityDrivenEaOptimizer().Optimize(problem,new InfeasibilityDrivenEaParameters { MaximumGenerations = 4, PopulationSize = 24 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<InfeasibilityDrivenEaOptimizer>(MetaheuristicFactory.Create<InfeasibilityDrivenEaOptimizer>(MetaheuristicAlgorithmIds.InfeasibilityDrivenEa));
 }
