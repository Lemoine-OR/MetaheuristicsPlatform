...
public sealed class GenocopIIIOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new GenocopIIIOptimizer().Optimize(problem,new GenocopIIIParameters { MaximumGenerations = 4, PopulationSize = 20, ReferencePopulationSize = 5 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<GenocopIIIOptimizer>(MetaheuristicFactory.Create<GenocopIIIOptimizer>(MetaheuristicAlgorithmIds.GenocopIII));
 }
