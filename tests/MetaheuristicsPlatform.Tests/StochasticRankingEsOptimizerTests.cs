...
public sealed class StochasticRankingEsOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new StochasticRankingEsOptimizer().Optimize(problem,new StochasticRankingEsParameters { MaximumGenerations = 4, Mu = 8, Lambda = 24 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<StochasticRankingEsOptimizer>(MetaheuristicFactory.Create<StochasticRankingEsOptimizer>(MetaheuristicAlgorithmIds.StochasticRankingEs));
 }
