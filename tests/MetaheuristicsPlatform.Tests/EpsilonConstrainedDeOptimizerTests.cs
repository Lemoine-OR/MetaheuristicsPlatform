...
public sealed class EpsilonConstrainedDeOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new EpsilonConstrainedDeOptimizer().Optimize(problem,new EpsilonConstrainedDeParameters { MaximumGenerations = 4, PopulationSize = 24, EpsilonControlGenerations = 3 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<EpsilonConstrainedDeOptimizer>(MetaheuristicFactory.Create<EpsilonConstrainedDeOptimizer>(MetaheuristicAlgorithmIds.EpsilonConstrainedDe));
 }
