using MetaheuristicsPlatform.Algorithms.BigBangBigCrunch;
...
public sealed class BBBCScientificTests
{
    [Fact]
    public void DescriptorAndFactoryUseCanonicalScientificIdentity()
    {
        var optimizer = new BigBangBigCrunchOptimizer();
        Assert.Equal(MetaheuristicAlgorithmIds.BigBangBigCrunch, optimizer.Descriptor.Id);
        Assert.Contains(optimizer.Descriptor.References, reference => reference.Doi == "10.1016/j.advengsoft.2005.04.005");
        Assert.NotNull(MetaheuristicFactory.Create<BigBangBigCrunchOptimizer>(MetaheuristicAlgorithmIds.BigBangBigCrunch));
    }
    ...
    public void OneCompleteIterationHasValidatedEvaluationAccounting()
    {
        OptimizationResult<double[]> result =
            new BigBangBigCrunchOptimizer().Optimize(
                CreateSphere(4),
                new BigBangBigCrunchParameters { PopulationSize = 6, MaximumIterations = 1 },
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new BigBangBigCrunchParameters { Alpha = 0.0 }.Validate());
    }

    private static OptimizationResult<double[]> Run(ulong seed) =>
        new BigBangBigCrunchOptimizer().Optimize(
            CreateSphere(5),
            new BigBangBigCrunchParameters { PopulationSize = 6, MaximumIterations = 2 },
            new ArraySolutionCloner<double>(),
            new NeverStoppingCriterion(),
