...
public sealed class DebConstraintGaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new DebConstraintGaOptimizer().Optimize(problem,new DebConstraintGaParameters { MaximumGenerations = 4 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<DebConstraintGaOptimizer>(MetaheuristicFactory.Create<DebConstraintGaOptimizer>(MetaheuristicAlgorithmIds.DebConstraintGa));
 }
