using MetaheuristicsPlatform.Algorithms.SineCosine;
...
public sealed class SCAScientificTests
{
    [Fact]
    public void DescriptorAndFactoryUseCanonicalScientificIdentity()
    {
        var optimizer = new SineCosineAlgorithmOptimizer();
        Assert.Equal(MetaheuristicAlgorithmIds.SineCosineAlgorithm, optimizer.Descriptor.Id);
        Assert.Contains(optimizer.Descriptor.References, reference => reference.Doi == "10.1016/j.knosys.2015.12.022");
        Assert.NotNull(MetaheuristicFactory.Create<SineCosineAlgorithmOptimizer>(MetaheuristicAlgorithmIds.SineCosineAlgorithm));
    }
    ...
    public void OneCompleteIterationHasValidatedEvaluationAccounting()
    {
        OptimizationResult<double[]> result =
            new SineCosineAlgorithmOptimizer().Optimize(
                CreateSphere(4),
                new SineCosineAlgorithmParameters { PopulationSize = 6, MaximumIterations = 1 },
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new SineCosineAlgorithmParameters { PopulationSize = 1 }.Validate());
    }

    private static OptimizationResult<double[]> Run(ulong seed) =>
        new SineCosineAlgorithmOptimizer().Optimize(
            CreateSphere(5),
            new SineCosineAlgorithmParameters { PopulationSize = 6, MaximumIterations = 2 },
            new ArraySolutionCloner<double>(),
            new NeverStoppingCriterion(),
