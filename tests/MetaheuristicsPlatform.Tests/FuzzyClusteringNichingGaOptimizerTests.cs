...
public sealed class FuzzyClusteringNichingGaOptimizerTests
{
    [Fact]
    public void Optimize_ReturnsDistinctOptima_AndFactoryCreatesCanonicalType()
    {
        ...
        MultimodalOptimizationResult result =
            new FuzzyClusteringNichingGaOptimizer().Optimize(
                problem,
                new FuzzyClusteringNichingGaParameters { MaximumGenerations = 4, PopulationSize = 24, ClusterCount = 4 },
        ...
        Assert.IsType<FuzzyClusteringNichingGaOptimizer>(
            MetaheuristicFactory.Create<FuzzyClusteringNichingGaOptimizer>(
