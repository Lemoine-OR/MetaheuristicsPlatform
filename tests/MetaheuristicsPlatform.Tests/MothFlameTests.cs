using MetaheuristicsPlatform.Algorithms.MothFlame;
...
public sealed class MFOScientificTests
{
    [Fact]
    public void DescriptorAndFactoryUseCanonicalScientificIdentity()
    {
        var optimizer = new MothFlameOptimizer();
        Assert.Equal(MetaheuristicAlgorithmIds.MothFlameOptimization, optimizer.Descriptor.Id);
        Assert.Contains(optimizer.Descriptor.References, reference => reference.Doi == "10.1016/j.knosys.2015.07.006");
        Assert.NotNull(MetaheuristicFactory.Create<MothFlameOptimizer>(MetaheuristicAlgorithmIds.MothFlameOptimization));
    }
    ...
    public void OneCompleteIterationHasValidatedEvaluationAccounting()
    {
        OptimizationResult<double[]> result =
            new MothFlameOptimizer().Optimize(
                CreateSphere(4),
                new MothFlameOptimizerParameters { PopulationSize = 6, MaximumIterations = 1 },
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new MothFlameOptimizerParameters { PopulationSize = 1 }.Validate());
    }

    private static OptimizationResult<double[]> Run(ulong seed) =>
        new MothFlameOptimizer().Optimize(
            CreateSphere(5),
            new MothFlameOptimizerParameters { PopulationSize = 6, MaximumIterations = 2 },
            new ArraySolutionCloner<double>(),
            new NeverStoppingCriterion(),
