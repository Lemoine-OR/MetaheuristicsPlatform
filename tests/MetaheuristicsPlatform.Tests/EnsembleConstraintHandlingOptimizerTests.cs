...
public sealed class EnsembleConstraintHandlingOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new EnsembleConstraintHandlingOptimizer().Optimize(problem,new EnsembleConstraintHandlingParameters { MaximumGenerations = 4, SubpopulationSize = 6, ExchangePeriod = 1 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<EnsembleConstraintHandlingOptimizer>(MetaheuristicFactory.Create<EnsembleConstraintHandlingOptimizer>(MetaheuristicAlgorithmIds.EnsembleConstraintHandling));
 }
