...
public sealed class DynamicNicheSharingGaOptimizerTests
{
    [Fact]
    public void Optimize_ReturnsDistinctOptima_AndFactoryCreatesCanonicalType()
    {
        ...
        MultimodalOptimizationResult result =
            new DynamicNicheSharingGaOptimizer().Optimize(
                problem,
                new DynamicNicheSharingGaParameters { MaximumGenerations = 4, PopulationSize = 24 },
        ...
        Assert.IsType<DynamicNicheSharingGaOptimizer>(
            MetaheuristicFactory.Create<DynamicNicheSharingGaOptimizer>(
