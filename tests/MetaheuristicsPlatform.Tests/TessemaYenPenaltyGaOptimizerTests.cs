...
public sealed class TessemaYenPenaltyGaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new TessemaYenPenaltyGaOptimizer().Optimize(problem,new TessemaYenPenaltyGaParameters { MaximumGenerations = 4 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<TessemaYenPenaltyGaOptimizer>(MetaheuristicFactory.Create<TessemaYenPenaltyGaOptimizer>(MetaheuristicAlgorithmIds.TessemaYenPenaltyGa));
 }
