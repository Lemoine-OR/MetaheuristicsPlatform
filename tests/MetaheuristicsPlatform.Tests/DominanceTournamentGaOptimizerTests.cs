...
public sealed class DominanceTournamentGaOptimizerTests
{
 [Fact] public void Optimize_UsesNativeConstraintContract_AndFactoryCreatesCanonicalType()
 {
   var problem=CreateProblem();
   ConstrainedOptimizationResult result=new DominanceTournamentGaOptimizer().Optimize(problem,new DominanceTournamentGaParameters { MaximumGenerations = 4 },new OptimizationOptions{Seed=11223344UL},cancellationToken: TestContext.Current.CancellationToken);
   ...
   Assert.IsType<DominanceTournamentGaOptimizer>(MetaheuristicFactory.Create<DominanceTournamentGaOptimizer>(MetaheuristicAlgorithmIds.DominanceTournamentGa));
 }
