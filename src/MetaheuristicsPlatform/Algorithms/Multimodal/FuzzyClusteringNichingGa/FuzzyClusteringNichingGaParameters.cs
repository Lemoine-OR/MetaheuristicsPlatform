...
public sealed class FuzzyClusteringNichingGaParameters : IMetaheuristicParameters
{
    ...
    public int ClusterCount { get; init; } = 6;
    public double Fuzziness { get; init; } = 2.0;
    public void Validate()
    {
        ...
        if (!double.IsFinite(Fuzziness) || Fuzziness <= 1.0)
            throw new ArgumentOutOfRangeException(nameof(Fuzziness));
    }
}
