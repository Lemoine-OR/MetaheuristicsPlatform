...
/// <summary>
/// High-performance classical Record-to-Record Travel optimizer.
/// Scientific basis: Dueck (1993), DOI 10.1006/jcph.1993.1010.
/// </summary>
public sealed class RecordToRecordTravelOptimizer<TSolution,TMove,TUndo> :
    IMetaheuristic<TSolution,RecordToRecordTravelParameters>
{
    ...
    public RecordToRecordTravelOptimizer(
    ...
    {
        Id = "record-to-record-travel-dueck-1993",
        Name = "Record-to-Record Travel",
        Acronym = "RRT",
    ...
    public RecordToRecordTravelParameters CreateDefaultParameters() => new();

    public OptimizationResult<TSolution> Optimize(
        IOptimizationProblem<TSolution> problem,
        RecordToRecordTravelParameters parameters,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var policy = new RecordToRecordTravelAcceptancePolicy(parameters.Deviation);
        ...
        var initialState = new RecordToRecordTravelState(
            double.NaN,problem.Sense.WorstValue(),parameters.Deviation,
            0,0,0,0,0,0,0,0);

        context.Start(initialState);
        double current = context.Evaluate(solution,initialState);

        RecordToRecordTravelState state = CreateState(
            current,context.State.BestFitness,parameters.Deviation,in statistics,failures);
    ...
    private static RecordToRecordTravelState CreateState(
        double current,
        double record,
        double deviation,
