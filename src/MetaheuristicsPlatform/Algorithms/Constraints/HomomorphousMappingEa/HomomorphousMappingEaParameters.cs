...
public sealed class HomomorphousMappingEaParameters : IMetaheuristicParameters
{
    ...
    public int MaximumReferenceAttempts { get; init; } = 5000;
    public int BisectionSteps { get; init; } = 30;
    public void Validate()
    {
        ...
        if (MaximumReferenceAttempts <= 0) throw new ArgumentOutOfRangeException(nameof(MaximumReferenceAttempts));
        if (BisectionSteps <= 0) throw new ArgumentOutOfRangeException(nameof(BisectionSteps));
    }
}
