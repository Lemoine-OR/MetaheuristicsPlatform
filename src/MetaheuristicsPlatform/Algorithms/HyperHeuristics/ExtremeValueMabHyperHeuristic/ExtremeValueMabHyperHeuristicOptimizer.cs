...
namespace MetaheuristicsPlatform.Algorithms.HyperHeuristics.ExtremeValueMabHyperHeuristic;

public sealed class ExtremeValueMabHyperHeuristicOptimizer :
    IHyperHeuristicOptimizer<ExtremeValueMabHyperHeuristicParameters>
{
    public MetaheuristicDescriptor Descriptor { get; } =
        new()
        {
            Id = MetaheuristicAlgorithmIds.ExtremeValueMabHyperHeuristic,
            Name = "Extreme-Value Dynamic Multi-Armed Bandit AOS",
            Acronym = "ExDMAB",
            ...
            References =
                new[]
                {
                    ExtremeValueMabHyperHeuristicOptimizerReferences.Primary
                }
        };

public HyperHeuristicOptimizationResult Optimize(
        IHyperHeuristicDomain domain,
        ExtremeValueMabHyperHeuristicParameters parameters,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 0; iteration < parameters.MaximumIterations; iteration++)
        {
            ...
            int selected = ExtremeUcbSelect(pulls, rewards, iteration, parameters.Exploration, random);
            ...
            pulls[selected]++;
            rewardWindows[selected].Enqueue(reward);
            while (rewardWindows[selected].Count > parameters.RewardWindow) rewardWindows[selected].Dequeue();
            rewards[selected] = ExtremeReward(rewardWindows[selected]);
    ...
    private static int ExtremeUcbSelect(IReadOnlyList<int> pulls, IReadOnlyList<double> rewards, int iteration, double exploration, IRandomSource random)
    {
    ...
    private static double ExtremeReward(IEnumerable<double> rewards) { return rewards.Any() ? rewards.Max() : 0.0; }
}
