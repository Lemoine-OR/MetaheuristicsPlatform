...
public sealed class ReinforcementLearningGreatDelugeHyperHeuristicParameters : IMetaheuristicParameters
{
    public int MaximumIterations { get; init; } = 400;
    public double LearningRate { get; init; } = 0.2;
    public double Exploration { get; init; } = 1.0;
    public double RainSpeed { get; init; } = 0.001;
    ...
    public void Validate()
    {
        ...
        if (!double.IsFinite(RainSpeed) || RainSpeed <= 0.0) throw new ArgumentOutOfRangeException(nameof(RainSpeed));
    }
}
