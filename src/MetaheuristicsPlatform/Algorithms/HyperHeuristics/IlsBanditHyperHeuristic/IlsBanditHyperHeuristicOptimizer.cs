...
public sealed class IlsBanditHyperHeuristicOptimizer :
    IHyperHeuristicOptimizer<IlsBanditHyperHeuristicParameters>
{
    public MetaheuristicDescriptor Descriptor { get; } =
        new()
        {
            Id = MetaheuristicAlgorithmIds.IlsBanditHyperHeuristic,
            Name = "ILS Hyper-Heuristic with Effective Heuristic Subset",
            Acronym = "ILS-HH",
            ...
            References =
                new[]
                {
                    IlsBanditHyperHeuristicOptimizerReferences.Primary
                }
        };

public HyperHeuristicOptimizationResult Optimize(
        IHyperHeuristicDomain domain,
        IlsBanditHyperHeuristicParameters parameters,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 0; iteration < parameters.MaximumIterations; iteration++)
        {
            ...
            int selected = SelectEffectiveSubset(pulls, rewards, parameters.SubsetSize, iteration, parameters.Exploration, random);
    ...
    private static int SelectEffectiveSubset(IReadOnlyList<int> pulls, IReadOnlyList<double> rewards, int subsetSize, int iteration, double exploration, IRandomSource random)
    {
        int[] subset = Enumerable.Range(0,rewards.Count).OrderByDescending(i => pulls[i]==0 ? double.PositiveInfinity : rewards[i]).Take(Math.Min(subsetSize,rewards.Count)).ToArray();
        foreach (int i in subset) if (pulls[i]==0) return i;
        double best=double.NegativeInfinity; int selected=subset[0];
        foreach (int i in subset) { double score=rewards[i]+exploration*Math.Sqrt(Math.Log(Math.Max(2,iteration+1))/pulls[i]); if(score>best){best=score;selected=i;} }
        return selected;
    }
}
