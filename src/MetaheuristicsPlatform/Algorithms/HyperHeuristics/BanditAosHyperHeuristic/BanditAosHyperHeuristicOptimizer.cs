...
public sealed class BanditAosHyperHeuristicOptimizer :
    IHyperHeuristicOptimizer<BanditAosHyperHeuristicParameters>
{
    public MetaheuristicDescriptor Descriptor { get; } =
        new()
        {
            Id = MetaheuristicAlgorithmIds.BanditAosHyperHeuristic,
            Name = "Bandit-Based Adaptive Operator Selection",
            Acronym = "MAB-AOS",
            ...
            References =
                new[]
                {
                    BanditAosHyperHeuristicOptimizerReferences.Primary
                }
        };

public HyperHeuristicOptimizationResult Optimize(
        IHyperHeuristicDomain domain,
        BanditAosHyperHeuristicParameters parameters,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 0; iteration < parameters.MaximumIterations; iteration++)
        {
            ...
            int selected = UcbSelect(pulls, rewards, iteration, parameters.Exploration, random);
    ...
    private static int UcbSelect(IReadOnlyList<int> pulls, IReadOnlyList<double> rewards, int iteration, double exploration, IRandomSource random)
    {
